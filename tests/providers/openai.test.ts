...
describe('OpenAI Provider', () => {
	...
	describe('isProviderAlive', () => {
		it('should return true with valid API key', async () => {
			const result = await openai.isProviderAlive(validApiKey, 'https://api.openai.com');
			expect(result).toBe(true);
		});

		it('should return false with invalid API key', async () => {
			const result = await openai.isProviderAlive(invalidApiKey, 'https://api.openai.com');
			expect(result).toBe(false);
		});

		it('should return false on network error', async () => {
			const result = await openai.isProviderAlive(validApiKey, 'https://nonexistent-api.com');
			expect(result).toBe(false);
		});
	...
	describe('listModels', () => {
		it('should return sorted list of model IDs with valid API key', async () => {
			const models = await openai.listModels(validApiKey, 'https://api.openai.com');
			
			expect(models).toEqual(['gpt-3.5-turbo', 'gpt-4', 'gpt-4-turbo']);
		});

		it('should return empty array with invalid API key', async () => {
			const models = await openai.listModels(invalidApiKey, 'https://api.openai.com');
			expect(models).toEqual([]);
		});

		it('should return empty array on network error', async () => {
			const models = await openai.listModels(validApiKey, 'https://nonexistent-api.com');
			expect(models).toEqual([]);
		});
	...
	describe('createProvider', () => {
		...
		it('should create provider with custom baseURL', () => {
			const provider = openai.createProvider(validApiKey, 'https://custom-api.com');
			expect(provider).toBeDefined();
		});
