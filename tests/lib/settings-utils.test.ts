...
import { maskApiKey, resolveApiKey, computeDisplayName } from '../../src/lib/settings-utils.ts';
import type { ModelConfiguration, ApiKeyConfiguration } from '../../src/ui/settings.ts';
...
describe('settings-utils', () => {
	...
	describe('resolveApiKey', () => {
		...
		it('should handle multiple API keys correctly', () => {
			...
			expect(resolveApiKey(config1, mockApiKeys)).toBe('sk-1234567890abcdef1234567890abcdef');
			expect(resolveApiKey(config2, mockApiKeys)).toBe('sk-or-v1-1234567890abcdef1234567890abcdef');
		});
	});
	...
	describe('computeDisplayName', () => {
		...
		it('should handle different providers correctly', () => {
			...
			expect(computeDisplayName('openrouter', 'claude-3-sonnet')).toBe('openrouter:claude-3-sonnet');
			expect(computeDisplayName('lmstudio', 'mistral-7b')).toBe('lmstudio:mistral-7b');
		});
		...
		it('should handle special characters in model names', () => {
			expect(computeDisplayName('openai', 'gpt-4-turbo-preview')).toBe('openai:gpt-4-turbo-preview');
			expect(computeDisplayName('ollama', 'llama2:13b-chat')).toBe('ollama:llama2:13b-chat');
		});

		it('should handle long model names', () => {
			const longModelName = 'very-long-model-name-with-many-characters-and-details';
	...
	describe('integration scenarios', () => {
		it('should work together for complete model resolution workflow', () => {
			...
			const computedName = computeDisplayName(config.provider, config.modelName);
